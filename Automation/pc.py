...
def gui():
    ...
    while True:
        ...
        with placeholder.container():
            ...
            kpi1.metric(label = "SCID \U0001F6F8", value = df.iloc[:,0])
            kpi2.metric(label = "AOS \u231B", value = str(df.iloc[:,1]))
            kpi3.metric(label = "LOS \u23F3 ", value = str(df.iloc[:,2]))
            kpi4.metric(label = "Ground Site \U0001F4E1", value = df.iloc[:,3])
            kpi5.metric(label = "Max Elevation [deg] \u26F0", value = df.iloc[:,4])
            kpi6.metric(label = "Duration [s] \u23F1", value = df.iloc[:,5])
            kpi7.metric(label = "Active \u2753", value = df.iloc[:,6])
            kpi8.metric(label = "Time Left \U0001F6D1", value = df.iloc[:,7])
