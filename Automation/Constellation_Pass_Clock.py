...
# Creating Fake Data - 18 Contacts
# JDAY, AOS, LOS, SCID GND MAXE - Leaving out Rev #
global data
...
for i in range(3,21): # 3 to 21 minutes ahead
    d = timedelta(minutes = i ) # can remove
    d1 = timedelta(minutes = i + random.randint(4,9)) # can remove

    jday_data.append((now+d).timetuple().tm_yday) # new jday
    aos_data.append(now + d) # aos/los are currently full dts 
...
# Contact Schedule Function - Actual part of the Constellation Pass Clock
def contact_schedule(num_contacts,scid_list):
    ...
    td = [] # time difference
    flag = [] # upcoming/active flag - using emoji unicodes
    dur = [] # contact duration in seconds
    data_final = [] # contacts that are upcoming/active

    for x in range(len(data)): # Checking all contacts
        ...
        if contact_scid in scid_list:
            ...
            if aos_full_pres.year == now.year and los_full_pres.year == now.year and\
               aos_full_pres.month == 1 and los_full_pres.month == 1 and now.month != 1: 
                yr_change = now + timedelta(days=4) # < 4 Days until a new schedule email is sent 
            ...
            if td_temp < 0:
                ...
                los_full_pres = dt(jd_change.year,jd_change.month,jd_change.day,\
                                   los_full_00.hour,los_full_00.minute,los_full_00.second)
            td_temp = (los_full_pres - aos_full_pres).total_seconds()
            ...
            print("Desired SCID doesn't have any upcoming/active contacts.")

    if num_contacts == 0 or num_contacts > len(data_final):
        num_contacts = len(data_final)
...
def gui():
    ...
    while True:
        ...
        with placeholder.container():
            ...
            if num_contacts == 0: # all of the contacts
                last8 = df.iloc[1:,:] 
            else: # if num_contact != 9 then its not actually last 8... I just usually have 9
                num_contacts -= 1
